...
RedisMgr::RedisMgr() {
  ...
  _con_pool.reset(
      new RedisConPool(5, host.c_str(), atoi(port.c_str()), pwd.c_str()));
}
...
bool RedisMgr::LPush(const std::string &key, const std::string &value) {
  ...
  auto _reply = (redisReply *)redisCommand(connect, "LPUSH %s %s", key.c_str(),
                                           value.c_str());
  if (NULL == _reply) {
...
bool RedisMgr::RPush(const std::string &key, const std::string &value) {
  ...
  auto _reply = (redisReply *)redisCommand(connect, "RPUSH %s %s", key.c_str(),
                                           value.c_str());
  if (NULL == _reply) {
...
bool RedisMgr::HSet(const std::string &key, const std::string &hkey,
  ...
  auto _reply = (redisReply *)redisCommand(
